...
var app = (function () {
    ...
    function genPickRandom(keysAndWeights) {
    ...
    function getImgUrl(offsetWidth, offsetHeight, html, css) {
    ...
    function getPixels(offsetWidth, offsetHeight, html, css) {
      return new Promise(resolve => {
    ...
    function matchImages(imgA, imgB) {
    ...
    async function extractCSSConsts(cssElement) {
    ...
    const absoluteCoverPosition = {
      type: 'position',
      value:`absolute; left:0; top:0; bottom:0; right:0`
    };

    function createGenerateRandomStyle({ sizes, colors, urls, classNames }) {
    ...
    function renderCSS(styles) {
      ...
      const stylesBySelectors = lodash.groupBy(styles, 'selector');
      const stylesTextBySelector = lodash.map(stylesBySelectors, (rules, selector) => {
        const stylesByType = lodash(rules)
          .groupBy('type')
        ...
        const rulesAsText = lodash.map(stylesByType, (rules, type) => `
      ${type}:${lodash.map(rules,'value').join(',')};`);
    ...
    async function evolvePool({pool, generation}, fitness, mutate) {
        generation += 1;
...
`;
    ...
    class GeneticCSS {
        ...
        async runEpoch(callback) {
            ...
            const best = lodash.first(this.population.pool);
            const worst = lodash.last(this.population.pool);
    ...
    function create_fragment(ctx) {
    	var div1, h1, t1, input0, t2, br0, t3, input1, t4, span, t5, t6, t7, t8, t9, br1, t10, section, t11, sidebar, canvas_1, t12, footer, raw1_value = `<style>${ctx.output}</style>`, raw1_after, t13, div0, t14, pre, t15, dispose;

    	return {
    		c: function create() {
    			div1 = element("div");
    			h1 = element("h1");
    			h1.textContent = "CSS Evolution!";
    			t1 = space();
    			...
    			pre = element("pre");
    			t15 = text(ctx.output);
    			add_location(h1, file, 76, 1, 1461);
    			attr(input0, "type", "checkbox");
    			add_location(input0, file, 77, 1, 1486);
    			add_location(br0, file, 78, 1, 1539);
    			attr(input1, "type", "checkbox");
    			add_location(input1, file, 79, 1, 1545);
    			add_location(span, file, 80, 1, 1596);
    			add_location(br1, file, 81, 1, 1660);
    			attr(section, "class", "svelte-nkc3bm");
    			add_location(section, file, 82, 1, 1666);
    			add_location(canvas_1, file, 86, 2, 1740);
    			add_location(sidebar, file, 85, 1, 1728);
    			set_style(div0, "padding", "20px");
    			attr(div0, "class", "svelte-nkc3bm");
    			add_location(div0, file, 90, 2, 1830);
    			add_location(pre, file, 93, 2, 1888);
    			attr(footer, "class", "svelte-nkc3bm");
    			add_location(footer, file, 88, 1, 1782);
    			attr(div1, "class", "svelte-nkc3bm");
    			add_location(div1, file, 75, 0, 1454);
    		...
    		p: function update(changed, ctx) {
    			if (changed.running) input0.checked = ctx.running;
    			if (changed.render) input1.checked = ctx.render;

    			if (changed.score) {
    				set_data(t6, ctx.score);
    			}

    			if (changed.currentGeneration) {
    				set_data(t8, ctx.currentGeneration);
    			}
    			...
    			if ((changed.output) && raw1_value !== (raw1_value = `<style>${ctx.output}</style>`)) {
    				detach_before(raw1_after);
    				raw1_after.insertAdjacentHTML("beforebegin", raw1_value);
    			}
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	let { html = '<div></div>', style = '', semantic = '<div></div>' } = $$props;
    	let canvas = null;
    	let render = true;
    	let running = false;
    	...
    	async function init (){
    		...
    		const gcss = new GeneticCSS(pixels => {
    			if (first) { 
    				canvas.style = `width:${pixels.width}px;height:${pixels.height}px;`; $$invalidate('canvas', canvas);
    				first = false;
    			}
    			if (render) {
    				const ctx = canvas.getContext('2d');
    		...
    		$$invalidate('genetic', genetic = gcss);
    	}

    	async function tick({best, scores, time, style}) {
    		if (style && style !== output) {
    			$$invalidate('output', output = style);
    			$$invalidate('score', score = best.score);
    		}
    		setTimeout(() => {
    			if (running) {
    				$$invalidate('currentGeneration', currentGeneration = currentGeneration + 1);
    				genetic.runEpoch(tick);
    			}
    		},0);
    	}

    	const writable_props = ['html', 'style', 'semantic'];
    	...
    	function input0_change_handler() {
    		...
    		$$invalidate('running', running);
    	}

    	function input1_change_handler() {
    		render = this.checked;
    		$$invalidate('render', render);
    	}
    	...
    	$$self.$set = $$props => {
    		...
    		if ('semantic' in $$props) $$invalidate('semantic', semantic = $$props.semantic);
    	};

    	let source;

    	$$self.$$.update = ($$dirty = { style: 1, html: 1, container: 1, semantic: 1, running: 1, genetic: 1 }) => {
    		if ($$dirty.style || $$dirty.html) { $$invalidate('source', source = `<style>${style}</style>${html}`); }
    		if ($$dirty.container || $$dirty.html || $$dirty.style || $$dirty.semantic) { container && html && style && semantic && init(); }
    		if ($$dirty.running || $$dirty.genetic) { running && genetic && tick({}); }
    	};
    ...
    class App extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, ["html", "style", "semantic"]);
    	}
