...
var app = (function () {
  ...
  `);
    ...
    function create_fragment(ctx) {
    	var div1, h1, t1, button, t3, br0, t4, input, t5, span, t6, t7_value = ctx.display && ctx.display.score, t7, t8, t9_value = ctx.display && ctx.pool.generation, t9, t10, br1, t11, section, t12, sidebar, canvas_1, t13, footer, raw1_after, t14, div0, t15, pre_1, t16, dispose;
    	...
    	return {
    		c: function create() {
    			...
    			pre_1 = element("pre");
    			t16 = text(ctx.best);
    			add_location(h1, file, 149, 1, 4152);
    			add_location(button, file, 150, 1, 4177);
    			add_location(br0, file, 151, 1, 4223);
    			attr(input, "type", "checkbox");
    			add_location(input, file, 152, 1, 4229);
    			add_location(span, file, 153, 1, 4280);
    			add_location(br1, file, 154, 1, 4372);
    			attr(section, "class", "svelte-nkc3bm");
    			add_location(section, file, 155, 1, 4378);
    			add_location(canvas_1, file, 159, 2, 4452);
    			add_location(sidebar, file, 158, 1, 4440);
    			set_style(div0, "padding", "20px");
    			attr(div0, "class", "svelte-nkc3bm");
    			add_location(div0, file, 163, 2, 4520);
    			add_location(pre_1, file, 166, 2, 4578);
    			attr(footer, "class", "svelte-nkc3bm");
    			add_location(footer, file, 161, 1, 4494);
    			attr(div1, "class", "svelte-nkc3bm");
    			add_location(div1, file, 148, 0, 4145);

    			dispose = [
    				listen(button, "click", ctx.toggleEvolve),
    		...
    		m: function mount(target, anchor) {
    			...
    			raw1_after.insertAdjacentHTML("beforebegin", ctx.best);
    			append(footer, t14);
    			append(footer, div0);
    		...
    		p: function update(changed, ctx) {
    			if (changed.render) input.checked = ctx.render;

    			if ((changed.display) && t7_value !== (t7_value = ctx.display && ctx.display.score)) {
    				set_data(t7, t7_value);
    			}

    			if ((changed.display || changed.pool) && t9_value !== (t9_value = ctx.display && ctx.pool.generation)) {
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	async function init (){
    		...
    		canvas.style = `width: ${offsetWidth + 40}px; height: ${offsetHeight + 40}px;`; $$invalidate('canvas', canvas);
    		...
    		$$invalidate('pool', pool = initPool(() => [ genStyle()]));
    		// pool = initPool(() => [genStyle()]);
    		$$invalidate('display', display = pool.pool[0]);
    	...
    	async function runEpoch() {
    		...
    		$$invalidate('pool', pool = await evolvePool(pool, fitnessFunc, mutateFunc ));
    		window.pool = pool;
    		$$invalidate('display', display = pool.pool[0]);
    	...
    	$$self.$$.update = ($$dirty = { style: 1, html: 1, display: 1 }) => {
    		if ($$dirty.style || $$dirty.html) { $$invalidate('source', source = `<style>${style}</style>${html}`); }
    		if ($$dirty.display) { $$invalidate('best', best = `<style>${pretty(pre + renderCSS(defaultPrePostPost.concat(display ? display.genes: [])))}</style>`); }
    	};
